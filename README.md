...
**Candidate only. Not published. Effective dates pending final publication.** Pushes to this feature branch are for review; do not merge into `main` or deploy Pages as part of this handoff. The live site's status and the available app are unchanged. No 1.0.9 availability or real-device acceptance is claimed.
...
| `delete-account/index.html` | Explicitly nonfunctional information preview: no sign-in, request submission, scripts or backend connection. Must be replaced before account-enabled publication. |
...
All pages use semantic HTML and relative links compatible with the `/the-daily-quest-site/` GitHub Pages subpath. There are no added scripts, trackers, fonts, dependencies or remote assets. Draft privacy, Terms and deletion pages use `noindex`. The archived page retains its original navigation exactly; its old iOS wording is historical text, not an announcement of iPhone distribution.
...
4. Replace the **entire** `delete-account/` information preview with the final generated authenticated deletion site and its assets from the qualified application revision. The app repository's `artifacts/deletion-site` was inspected but **not copied or exercised**. Do not combine a stale generated index with different hashed assets. Verify Google-only sign-in, exact callback, cancellation, reauthentication, accepted/lost responses, deletion retries and stale-write rejection using disposable accounts, without requiring app reinstallation. Retain useful local-only help/navigation in the qualified experience if appropriate.
...
The coordinator owns app/backend/cloud qualification, final substitutions, `main` publication and live checks. This branch owns static candidate HTML/CSS/documentation only.
